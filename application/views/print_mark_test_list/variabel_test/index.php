...
<div id="print_mark_test_list_print_mark_test_list_detail_toolbar" style="padding-bottom: 2px;">
    <?php
    if (in_array("Add", $action)) {
        ?>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-add" plain="true" onclick="print_mark_test_list_print_mark_test_list_detail_add()"> Add</a>
        <?php
    }
    if (in_array("Edit", $action)) {
        ?>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="print_mark_test_list_print_mark_test_list_detail_edit()"> Edit</a>
        <?php
    }
    if (in_array("Delete", $action)) {
        ?>
        <a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="print_mark_test_list_print_mark_test_list_detail_delete()">Delete</a>
        <?php
    }
    ?>
</div>
<table  id="print_mark_test_list_print_mark_test_list_detail" data-options="
       url:'<?php echo site_url('print_mark_test_list/print_mark_test_list_detail_get') ?>',
       method:'post',
       border:true,
       title:'Variabel Test',
       singleSelect:true,
       ...
       striped:true,
       autoRowHeight:true,
       toolbar:'#print_mark_test_list_print_mark_test_list_detail_toolbar'">
    <thead>
        <tr>
...
<script type="text/javascript">
    $(function () {
        $('#print_mark_test_list_print_mark_test_list_detail').datagrid({
            ...
            onDblClickRow: function (rowIndex, row) {
              // print_mark_test_list_print_mark_test_list_detail_edit();
            }
        });
    });
    ...
    function showimage_product_test(value, row) {
        ...
        if (row.image_file == null)
            ...
            var temp = "<img src='files/hardnesstest/" + row.print_mark_test_list_id + "/" + row.image_file + "' width=90 height=90 onclick='print_mark_test_list_variabel_test_view_detail(" + idrow + ")'>";
    ...
    function showimage_product_test2(value, row) {
        ...
        if (row.image2_file == null)
            ...
            //var temp=row.image_file;
            var temp = "<img src='files/hardnesstest/" + row.print_mark_test_list_id + "/" + row.image2_file + "' width=90 height=90 onclick='print_mark_test_list_variabel_test_view_detail(" + idrow + ")'>";
            //var temp = "<img src='files/hardnesstest/" + row.print_mark_test_list_id + "/" + row.image_file + "' width=50>" + row.image_file;
        }
        return temp;
    }
    function showimage_product_test3(value, row) {
        ...
        if (row.image3_file == null)
            ...
            //var temp=row.image_file;
            var temp = "<img src='files/hardnesstest/" + row.print_mark_test_list_id + "/" + row.image3_file + "' width=90 height=90 onclick='print_mark_test_list_variabel_test_view_detail(" + idrow + ")'>";
            //var temp = "<img src='files/hardnesstest/" + row.print_mark_test_list_id + "/" + row.image_file + "' width=50>" + row.image_file;
        }
        return temp;
    }
    function formatDetail2_dt(value, row) {
        ...
        if (row.submited == 'f' || row.submited==null) {
            if (row.var_type == 'Photo')
                var temp = "<input type=button value='Upload Photo' id='dtvt" + row.id + "' onclick='print_mark_test_list_variabel_test_add(\"photo\")'> ";
            else
                var temp = "<input type=button value='Edit Notes' id='dtvt" + row.id + "' onclick='print_mark_test_list_variabel_test_add(\"notes\")'> ";
        } else
            var temp = 'submited';
        return temp;
    }
